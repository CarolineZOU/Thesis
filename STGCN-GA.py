...
def genetic_algorithm(fitness_func, param_ranges, pop_size=15, generations=10):
    if pop_size % 2 != 0:
        pop_size += 1
    ...
    for _ in range(generations):
        ...
        # 确保权重总和大于零
        valid_scores = [max(0, score) for score in fitness_scores]
        total_score = sum(valid_scores)
        if total_score <= 0:
            total_score = 1e-6  # 设置最小正值

        # 使用归一化权重
        weights = [score / total_score for score in valid_scores]
        ...
        for ind in new_population:
            if random.random() < 0.2:
                ...
                if idx == 0:
                    ...
                    ind[idx] = random.uniform(param_ranges[['dropout_rate', 'lr'][idx - 1]][0],
                                             param_ranges[['dropout_rate', 'lr'][idx - 1]][1])
...
if __name__ == "__main__":
    ...
    SAVE_DIR = "STGCN_GA_models"
    OUTPUT_DIR = "STGCN_GA_output"
    ...
    for city in cities:
        ...
        def fitness_function(params):
            ...
            with torch.no_grad():
                ...
                mse = criterion(pred, y_valid).item()
            # 返回正值适应度（修复）
            return 1.0 / (mse + 1e-6)
